...
public class ScoreSpecialController {
    ...
    public Result addScore(@RequestBody AddScoreDTO addClassDTO) {
        ...
        if (StringUtils.hasText(addClassDTO.getScore())
            ...
            scoreService.addScore(addClassDTO);
    ...
    public Result<PageResult<ScoreVO>> pageWarehouse(@RequestBody ScorePageQueryDTO scorePageQueryDTO){
        ...
        try{
            PageResult<ScoreVO> pageResult= scoreService.pageScore(scorePageQueryDTO);
    ...
    public Result updateWarehouse(@RequestBody UpdateStudentScoreDTO updateStudentScoreDTO) {
        scoreService.updatestudentScore(updateStudentScoreDTO);
    ...
    public Result deleteStudentScore(@PathVariable @Parameter(description = "要删除的学生的id") Long id) {
        ...
        try {
            scoreService.deleteStudentScore(id);
    ...
    public Result<ScoreVO> getWarehouseGoodsById(@PathVariable Long id) throws Exception {
        ...
        {
            //根据用户id去查找详情信息
            ScoreVO scoreVO= scoreService.getStudentScoreById(id);
    ...
    public Result<PageResult<ClassStudentScoreVO>> pageClassScore(@RequestBody PageQueryClassStudentScoreDTO pageQueryClassStudentScoreDTO){
        ...
        try{
            PageResult<ClassStudentScoreVO> pageResult= scoreService.pageCLassStudentScore(pageQueryClassStudentScoreDTO);
            return Result.success("查询成功",pageResult);
        }catch (BaseException e){
            return Result.error(e.getMessage());
        }
    ...
    public Result<PageResult<MyScoreVO>> pageWarehouse(@RequestBody MyScorePageQueryDTO myScorePageQueryDTO){
        log.info("开始分页查询我的成绩");
        try{
            PageResult<MyScoreVO> pageResult= scoreService.pageMyScore(myScorePageQueryDTO);
            return Result.success("查询成功",pageResult);
        }catch (BaseException e){
            return Result.error(e.getMessage());
        }
     ...
     * @return
     */
    @GetMapping("/classScoreAverageScore")
    @Operation(summary = "查询班级平均分，最高分，最低分")
    @ApiOperationSupport(author = "陈嘉豪")
    public Result<List<ClassAvgVO>> getClassAvg() {
        return Result.success("查询成功", scoreService.getClassAvg());
    }
    /**
     * 用Excel导入成绩
     */
    @PostMapping("/importExcelScore")
    @Operation(summary = "用Excel导入成绩")
    @ApiOperationSupport(author = "陈嘉豪")
